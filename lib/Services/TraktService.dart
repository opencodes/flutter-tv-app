...
class TraktService {
  ...
  Future<List<TraktModel>> getMovies(int limit) async {
    print('${baseUrl}/movies/trending?extended=full&limit=${limit}.toString()');
    var res = await http.get('${baseUrl}', headers: {'trakt-api-version': apiVersion, 'trakt-api-key': apiKey});
    if (res.statusCode == 200) {
      List<dynamic> body = jsonDecode(res.body);
      
      List<TraktModel> movies = mock_trending
          .map(
  ...
  Future<List<TraktModel>> getShows(int limit) async {
    print('${baseUrl}/shows/trending?extended=full&limit=${limit.toString()}');
    var res = await http.get('${baseUrl}', headers: {'trakt-api-version': apiVersion, 'trakt-api-key': apiKey});
    if (res.statusCode == 200) {
      List<dynamic> body = jsonDecode(res.body);
      List<TraktModel> shows = mock_shows
          .map(
  ...
  Future<List<TraktModel>> getSeasons(TraktModel show) async {
    print('${baseUrl}/shows/${show.ids['trakt']}/seasons?extended=episodes,full');
    var res = await http.get('${baseUrl}', headers: {'trakt-api-version': apiVersion, 'trakt-api-key': apiKey});
    if (res.statusCode == 200) {
      List<dynamic> body = jsonDecode(res.body);
